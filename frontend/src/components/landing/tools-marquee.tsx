...
interface Tool {
  name: string
  /** slug SimpleIcons en priorité. */
  slug: string
  /** slug SVGL.app si différent ou si le logo n'existe pas sur SimpleIcons. */
  svglSlug?: string
}

// Slugs SimpleIcons (https://simpleicons.org). On garde SVGL.app comme
// fallback pour les marques retirées du catalogue SimpleIcons (politique
// trademarks). Si les deux échouent → fallback texte (nom de la marque).
const TOOLS: Tool[] = [
  { name: 'ChatGPT', slug: 'openai' },
  { name: 'Claude', slug: 'claude', svglSlug: 'claude-ai' },
  { name: 'Gemini', slug: 'googlegemini', svglSlug: 'gemini' },
...
function ToolTile({
  ...
  // Cascade de sources : SimpleIcons d'abord (CSP-friendly, fiable),
  // puis SVGL.app si SimpleIcons n'a pas le logo, puis texte si tout
  // échoue. Step indique l'index courant dans la cascade.
  const sources = [
    `https://cdn.simpleicons.org/${tool.slug}`,
    `https://svgl.app/library/${tool.svglSlug ?? tool.slug}.svg`,
  ]
    ...
    >
      <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl border border-[var(--border)] bg-[var(--background)] transition-all duration-500 group-hover:scale-110 group-hover:border-[var(--primary)]/20 group-hover:bg-white group-hover:shadow-[0_12px_24px_-8px_rgba(0,0,0,0.08)] sm:h-20 sm:w-20">
        {step < sources.length ? (
          ...
          <span className="text-[10px] font-black uppercase tracking-tight text-[var(--muted-foreground)] sm:text-xs">
            {tool.name}
          </span>
        )}
      </div>
