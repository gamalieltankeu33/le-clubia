...
type PlanId = 'semestrial' | 'trimestrial' | 'trial'
...
serve(async (req: Request) => {
  ...
  const productIds: Record<PlanId, string | undefined> = {
    semestrial: Deno.env.get('MAKETOU_PRODUCT_ID_ANNUAL'),
    trimestrial: Deno.env.get('MAKETOU_PRODUCT_ID_SEMESTRIAL'),
  ...
  if (
    ...
    return json({ error: 'planId invalide (attendu : semestrial | trimestrial | trial).' }, 400)
  }
